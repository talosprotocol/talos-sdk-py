...
class SigningVerifyHandler(BaseHandler):
    ...
    def _test_verify(self, inputs, expected):
        ...
        signature = base64url_decode(inputs["signature_base64url"]) if "signature_base64url" in inputs else None
        ...
        if "unordered" in inputs:
            ...
            if "canonical_number" in expected:
                if res != expected["canonical_number"]:
                    raise AssertionError(f"Number encoding mismatch: {res} != {expected['canonical_number']}")
            elif "canonical" in expected:
                if res != expected["canonical"]:
                    raise AssertionError(f"Value canonical mismatch: {res} != {expected['canonical']}")
    ...
    def run_vector(self, vector):
        ...
        for key, val in expected.items():
            if key.startswith("authorize_fast_"):
                ...
                if result != val:
                    raise AssertionError(f"Authorization mismatch for {tool}:{action}. Got {result}")
...
class RatchetHandler(BaseHandler):
    def run_trace(self, trace):
        ...
        bob_mgr._signed_prekey = KeyPair(private_key=spk_priv, public_key=spk_pub, key_type="x25519")
        ...
        b_bundle_obj = PrekeyBundle(
             ...
             one_time_prekey=base64url_decode(bob_bundle["one_time_prekey"]) if bob_bundle["one_time_prekey"] else None
        )
        ...
        for step in trace["steps"]:
            ...
            try:
                if action == "encrypt":
                     ...
                     expected_pt = base64url_decode(step["expected_plaintext"] if "expected_plaintext" in step else step["plaintext"])
...
class MicroVectorHandler(BaseHandler):
    ...
    def _test_header_canonical(self, vector):
        ...
        if canonical != expected:
            raise AssertionError(f"Canonical mismatch. Got {base64url_encode(canonical)}, expected {vector['expected_canonical_b64u']}")
    ...
    def _test_kdf_rk(self, vector):
        ...
        if new_rk != expected_rk:
             raise AssertionError(f"New RK mismatch. Got {base64url_encode(new_rk)}, expected {vector['expected']['new_rk']}")
        if new_ck != expected_ck:
             raise AssertionError(f"New CK mismatch. Got {base64url_encode(new_ck)}, expected {vector['expected']['new_ck']}")
    ...
    def _test_kdf_ck(self, vector):
        ...
        if next_ck != expected_next_ck:
             raise AssertionError(f"Next CK mismatch. Got {base64url_encode(next_ck)}, expected {vector['expected']['next_ck']}")
        if mk != expected_mk:
             raise AssertionError(f"MK mismatch. Got {base64url_encode(mk)}, expected {vector['expected']['mk']}")
...
def get_handler_for_file(filename):
    ...
    if filename in ["roundtrip_basic.json", "out_of_order.json", "max_skip.json", "v1_1_0_roundtrip.json"]:
